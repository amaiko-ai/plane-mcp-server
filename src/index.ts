...
async function planeRequest(method: string, path: string, body: unknown = null): Promise<any> {
    ...
    tools: [
      {
        ...
        description: "List all intake items for a project. Returns pending, accepted, declined, and duplicate items.",
        inputSchema: {
          type: "object",
          properties: {
            project_id: {
            ...
            status: {
              type: "string",
              enum: ["-2", "-1", "1", "2"],
              description: "Optional filter by status: -2=pending, -1=declined, 1=accepted, 2=duplicate"
            }
          },
          required: ["project_id"]
        }
      },
      {
        ...
        description: "Accept an intake item, converting it to a regular issue. Can optionally add labels.",
        inputSchema: {
          type: "object",
          properties: {
            project_id: {
  ...
  try {
    switch (name) {
      case "list_intake_items": {
        ...
        const data = await planeRequest('GET', `workspaces/${WORKSPACE_SLUG}/projects/${project_id}/intake-issues/`);

        let items = data.results;
        if (status) {
          items = items.filter((i: any) => i.status === parseInt(status));
        }
          ...
          content: [{
            ...
            text: JSON.stringify({
              ...
              items: items.map((i: any) => ({
                ...
                status: i.status,
                status_text: i.status === -2 ? 'pending' : i.status === -1 ? 'declined' : i.status === 1 ? 'accepted' : 'duplicate',
      ...
      case "accept_intake_item": {
        ...
        await planeRequest('PATCH', `workspaces/${WORKSPACE_SLUG}/projects/${project_id}/intake-issues/${issue_id}/`, {
          status: 1
        });

        // If labels provided, add them to the converted issue
        if (label_ids && label_ids.length > 0) {
          await planeRequest('PATCH', `workspaces/${WORKSPACE_SLUG}/projects/${project_id}/issues/${issue_id}/`, {
            labels: label_ids
          });
        }
      ...
      case "mark_intake_duplicate": {
        ...
        await planeRequest('PATCH', `workspaces/${WORKSPACE_SLUG}/projects/${project_id}/intake-issues/${duplicate_issue_id}/`, {
          status: 2,
